defmodule BankWeb.EmployeeAuthTest do
  ...
  describe "log_in_employee/3" do
    test "stores the employee token in the session", %{conn: conn, employee: employee} do
      ...
      assert Admin.get_employee_by_session_token(token)
    end
  ...
  describe "logout_employee/1" do
    test "erases session and cookies", %{conn: conn, employee: employee} do
      employee_token = Admin.generate_employee_session_token(employee)
      ...
      assert redirected_to(conn) == "/"
      refute Admin.get_employee_by_session_token(employee_token)
    end
