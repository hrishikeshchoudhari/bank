defmodule Bank.AdminTest do
  ...
  alias Bank.Admin.{Employee, EmployeeToken}

  describe "get_employee_by_email/1" do
    test "does not return the employee if the email does not exist" do
      refute Admin.get_employee_by_email("unknown@example.com")
    end

    test "returns the employee if the email exists" do
      %{id: id} = employee = employee_fixture()
      assert %Employee{id: ^id} = Admin.get_employee_by_email(employee.email)
    end
  end

  describe "get_employee_by_email_and_password/2" do
    test "does not return the employee if the email does not exist" do
      refute Admin.get_employee_by_email_and_password("unknown@example.com", "hello world!")
    end

    test "does not return the employee if the password is not valid" do
      employee = employee_fixture()
      refute Admin.get_employee_by_email_and_password(employee.email, "invalid")
    end
    ...
    test "returns the employee if the email and password are valid" do
      ...
      assert %Employee{id: ^id} =
               Admin.get_employee_by_email_and_password(employee.email, valid_employee_password())
    end
  end
  ...
  describe "get_employee!/1" do
    test "raises if id is invalid" do
      assert_raise Ecto.NoResultsError, fn ->
        Admin.get_employee!(-1)
      end
    end

    test "returns the employee with the given id" do
      %{id: id} = employee = employee_fixture()
      assert %Employee{id: ^id} = Admin.get_employee!(employee.id)
    end
  end

  describe "register_employee/1" do
    test "requires email and password to be set" do
      {:error, changeset} = Admin.register_employee(%{})
    ...
    test "validates email and password when given" do
      {:error, changeset} = Admin.register_employee(%{email: "not valid", password: "not valid"})
    ...
    test "validates maximum values for email and password for security" do
      ...
      {:error, changeset} = Admin.register_employee(%{email: too_long, password: too_long})
    ...
    test "validates email uniqueness" do
      ...
      {:error, changeset} = Admin.register_employee(%{email: email})
      assert "has already been taken" in errors_on(changeset).email

      # Now try with the upper cased email too, to check that email case is ignored.
      {:error, changeset} = Admin.register_employee(%{email: String.upcase(email)})
      assert "has already been taken" in errors_on(changeset).email
    end

    test "registers employees with a hashed password" do
      email = unique_employee_email()
      {:ok, employee} = Admin.register_employee(valid_employee_attributes(email: email))
  ...
  describe "change_employee_registration/2" do
    test "returns a changeset" do
      assert %Ecto.Changeset{} = changeset = Admin.change_employee_registration(%Employee{})
    ...
    test "allows fields to be set" do
      ...
      changeset =
        Admin.change_employee_registration(
          %Employee{},
  ...
  describe "change_employee_email/2" do
    test "returns a employee changeset" do
      assert %Ecto.Changeset{} = changeset = Admin.change_employee_email(%Employee{})
  ...
  describe "apply_employee_email/3" do
    ...
    test "requires email to change", %{employee: employee} do
      {:error, changeset} = Admin.apply_employee_email(employee, valid_employee_password(), %{})
      assert %{email: ["did not change"]} = errors_on(changeset)
    end

    test "validates email", %{employee: employee} do
      {:error, changeset} =
        Admin.apply_employee_email(employee, valid_employee_password(), %{email: "not valid"})
    ...
    test "validates maximum value for email for security", %{employee: employee} do
      ...
      {:error, changeset} =
        Admin.apply_employee_email(employee, valid_employee_password(), %{email: too_long})
    ...
    test "validates email uniqueness", %{employee: employee} do
      ...
      {:error, changeset} =
        Admin.apply_employee_email(employee, valid_employee_password(), %{email: email})
    ...
    test "validates current password", %{employee: employee} do
      {:error, changeset} =
        Admin.apply_employee_email(employee, "invalid", %{email: unique_employee_email()})
    ...
    test "applies the email without persisting it", %{employee: employee} do
      email = unique_employee_email()
      {:ok, employee} = Admin.apply_employee_email(employee, valid_employee_password(), %{email: email})
      assert employee.email == email
      assert Admin.get_employee!(employee.id).email != email
    end
  end
    ...
    test "sends token through notification", %{employee: employee} do
      token =
        extract_employee_token(fn url ->
          Admin.deliver_update_email_instructions(employee, "current@example.com", url)
        end)
  ...
  describe "update_employee_email/2" do
    setup do
      ...
      token =
        extract_employee_token(fn url ->
          Admin.deliver_update_email_instructions(%{employee | email: email}, employee.email, url)
        end)
    ...
    test "updates the email with a valid token", %{employee: employee, token: token, email: email} do
      assert Admin.update_employee_email(employee, token) == :ok
    ...
    test "does not update email with invalid token", %{employee: employee} do
      assert Admin.update_employee_email(employee, "oops") == :error
    ...
    test "does not update email if employee email changed", %{employee: employee, token: token} do
      assert Admin.update_employee_email(%{employee | email: "current@example.com"}, token) == :error
      assert Repo.get!(Employee, employee.id).email == employee.email
      assert Repo.get_by(EmployeeToken, employee_id: employee.id)
    end
    ...
    test "does not update email if token expired", %{employee: employee, token: token} do
      ...
      assert Admin.update_employee_email(employee, token) == :error
      assert Repo.get!(Employee, employee.id).email == employee.email
      assert Repo.get_by(EmployeeToken, employee_id: employee.id)
    end
  end
  ...
  describe "change_employee_password/2" do
    test "returns a employee changeset" do
      assert %Ecto.Changeset{} = changeset = Admin.change_employee_password(%Employee{})
      assert changeset.required == [:password]
    end

    test "allows fields to be set" do
      changeset =
        Admin.change_employee_password(%Employee{}, %{
  ...
  describe "update_employee_password/3" do
    ...
    test "validates password", %{employee: employee} do
      {:error, changeset} =
        Admin.update_employee_password(employee, valid_employee_password(), %{
          password: "not valid",
    ...
    test "validates maximum values for password for security", %{employee: employee} do
      ...
      {:error, changeset} =
        Admin.update_employee_password(employee, valid_employee_password(), %{password: too_long})
    ...
    test "validates current password", %{employee: employee} do
      {:error, changeset} =
        Admin.update_employee_password(employee, "invalid", %{password: valid_employee_password()})
    ...
    test "updates the password", %{employee: employee} do
      ...
      assert is_nil(employee.password)
      assert Admin.get_employee_by_email_and_password(employee.email, "new valid password")
    end

    test "deletes all tokens for the given employee", %{employee: employee} do
      _ = Admin.generate_employee_session_token(employee)
  ...
  describe "get_employee_by_session_token/1" do
    ...
    test "returns employee by token", %{employee: employee, token: token} do
      assert session_employee = Admin.get_employee_by_session_token(token)
      assert session_employee.id == employee.id
    end

    test "does not return employee for invalid token" do
      refute Admin.get_employee_by_session_token("oops")
    end

    test "does not return employee for expired token", %{token: token} do
      {1, nil} = Repo.update_all(EmployeeToken, set: [inserted_at: ~N[2020-01-01 00:00:00]])
      refute Admin.get_employee_by_session_token(token)
    end
  end
  ...
  describe "confirm_employee/1" do
    ...
    test "confirms the email with a valid token", %{employee: employee, token: token} do
      assert {:ok, confirmed_employee} = Admin.confirm_employee(token)
    ...
    test "does not confirm with invalid token", %{employee: employee} do
      assert Admin.confirm_employee("oops") == :error
    ...
    test "does not confirm email if token expired", %{employee: employee, token: token} do
      {1, nil} = Repo.update_all(EmployeeToken, set: [inserted_at: ~N[2020-01-01 00:00:00]])
      assert Admin.confirm_employee(token) == :error
      refute Repo.get!(Employee, employee.id).confirmed_at
      assert Repo.get_by(EmployeeToken, employee_id: employee.id)
    end
  end
  ...
  describe "deliver_employee_reset_password_instructions/2" do
    ...
    test "sends token through notification", %{employee: employee} do
      token =
        extract_employee_token(fn url ->
          Admin.deliver_employee_reset_password_instructions(employee, url)
        end)
  ...
  describe "reset_employee_password/2" do
    ...
    test "validates password", %{employee: employee} do
      {:error, changeset} =
        Admin.reset_employee_password(employee, %{
          password: "not valid",
          password_confirmation: "another"
        })
    ...
    test "validates maximum values for password for security", %{employee: employee} do
      too_long = String.duplicate("db", 100)
      {:error, changeset} = Admin.reset_employee_password(employee, %{password: too_long})
      assert "should be at most 72 character(s)" in errors_on(changeset).password
    end

    test "updates the password", %{employee: employee} do
      {:ok, updated_employee} = Admin.reset_employee_password(employee, %{password: "new valid password"})
    ...
    test "deletes all tokens for the given employee", %{employee: employee} do
      _ = Admin.generate_employee_session_token(employee)
      {:ok, _} = Admin.reset_employee_password(employee, %{password: "new valid password"})
      refute Repo.get_by(EmployeeToken, employee_id: employee.id)
    end
  end
